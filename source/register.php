...
<!--    <div style="display: flex; justify-content: center; align-items: center;">-->
    <?php
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            if (isset($_POST['email']) && isset($_POST['phone']) && isset($_POST['user']) && isset($_POST['pass']) && isset($_POST['confirm_pass'])){
                $flag = true;
                $email = $_POST['email'];
                $phone = $_POST['phone'];
                $user = $_POST['user'];
                ...
                $stmt = $conn->query("SELECT email FROM users WHERE email='$email'");

                if ($stmt->rowCount() > 0) {
                    $GLOBALS['state'] = "failure";
                    $GLOBALS['message'] = "Account already exists";
                }
                else {
                    ...
                    if ($stmt->execute()) {
                        header("Location:sendOTP.php?email=".$email."&state=success&message=Registration Successfully");
                    }else {
                        $GLOBALS['state'] = "failure";
                        $GLOBALS['message'] = "Registration failed";
                    }
                }
            }
        }
